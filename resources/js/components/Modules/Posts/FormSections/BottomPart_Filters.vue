<template>
	<div style="width:47%" class="div-table-col row-card-white">
		...
		<div class="filterContainer " style="padding-bottom:65px;padding-top:10px;">
			<div
				v-for="(filter, index) in filters"
				:key="'filterId_' + index "
				:class="'row px-5 pt-4 pt-2 fs-13 '"
			>
				<div class="col-sm-3 col-md-3 col-lg-3 col-xl-3 text-left px-0">{{ filter.postFilterName }}</div>
				<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9">
					...
					<div
						v-if="form.filterForm['postFiltersId:'+filter.postFiltersId + '_postFilterName:'+filter.postFilterName.replace(/\s/g,'') ] == 3"
					>
						<div
				...
				<hr v-if="index < filters.length - 1 ">
			</div>
		</div>
	</div>
</template>
